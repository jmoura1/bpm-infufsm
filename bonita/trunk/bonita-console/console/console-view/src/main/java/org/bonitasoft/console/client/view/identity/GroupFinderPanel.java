 ...
 * 
 */
public class GroupFinderPanel extends Composite implements Focusable, BonitaDataSource<BonitaUUID> {

  public static final String CANCEL_PROPERTY = "group finder cancel";
  ...
  protected final GroupFilter myItemFilter = new GroupFilter(0, 10);
  protected AsyncHandler<ItemUpdates<Group>> myUserHandler;
  ...
  protected final GroupDataSource myInternalItemDataSource = new GroupDataSourceImpl(null);

  protected final transient ModelChangeSupport myChanges = new ModelChangeSupport(this);

  public GroupFinderPanel(boolean isMultiple) {
    super();
  ...
  protected void setSelectedItems(ItemSelection<BonitaUUID> anItemSelection) {
    ...
    if (anItemSelection != null) {
      ...
      if (validate()) {
        ...
        if (mySelectionIsMultiple) {
          myErrorMessageLabel.setHTML(constants.selectSomeGroups());
        } else {
          myErrorMessageLabel.setHTML(constants.selectExactlyOneGroup());
        }
      }
    }
  }
  ...
  public List<Group> getGroups() {
    final List<Group> theResult = new ArrayList<Group>();
