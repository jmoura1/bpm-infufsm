...
@ClientInterceptor
@Provider
public class RESTClientExecutionInterceptor implements
  ...
  public ClientResponse execute(ClientExecutionContext ctx) throws Exception {
    ...
    MapStringConverter converter = new MapStringConverter();
    
    Map<String, String> optionsMap = (Map<String, String>) converter.fromString(options.get(0));
    String userName = optionsMap.get(APIAccessor.REST_USER_OPTION);
    if (userName != null && !userName.trim().equals("")) {
      String passWord = optionsMap.get(APIAccessor.PASSWORD_HASH_OPTION);
    ...
    if (options != null && options.size() > 0){
      ...
      try {
        ...
        if(LOG.isLoggable(Level.WARNING)) {
          LOG.warning("Imposible to encode some parameters using UTF-8: " + e);
        }
      }
