...
public class TransientData {
  ...
  public static void addTransientVariable(ActivityInstanceUUID activityInstanceUUID, String variableName, Object variableValue){
    ...
    Map<ActivityInstanceUUID, Map<String, Object>> data = getTransientData();
  ...
  public static Object getActivityTransientVariableValue(ActivityInstanceUUID activityInstanceUUID, String variableName) throws VariableNotFoundException{
    Misc.checkArgsNotNull(activityInstanceUUID, variableName);
    ensureTransientVariablesCreated(activityInstanceUUID);
    Map<String, Object> variables = getActivityTransientVariables(activityInstanceUUID);
  ...
  private static void restoreTransientVariablesIfNecessary(final ActivityInstanceUUID activityInstanceUUID) {
    ...
    if(activityInstance != null && needRestore(activityInstance.getState())) {
      final InternalActivityDefinition activityDefinition = EnvTool.getJournalQueriers().getActivity(activityInstance.getActivityDefinitionUUID());
      final Map<String, Object> transientVariables = VariableUtil.createTransientVariables(activityDefinition.getDataFields(), activityInstance.getProcessInstanceUUID());
  ...
  private static boolean needRestore(final ActivityState state) {
    return ActivityState.EXECUTING.equals(state) || ActivityState.READY.equals(state) || ActivityState.SUSPENDED.equals(state);
  }
}
