 ...
 *
 */
public class RESTHttpURLConnectionManagementAPITest extends RESTHttpURLConnectionAPITestCase{
	public void testCheckUserCredentials() throws Exception {
		//valid user/password
		HttpURLConnection response = RESTHttpURLConnectionManagementAPI.checkUserCredentials("admin", "bpm", getUserForTheOptionsMap());
    ...
    response = RESTHttpURLConnectionManagementAPI.checkUserCredentials("admin", "invalidPassword", null);
    assertEquals(HttpURLConnection.HTTP_OK, response.getResponseCode());
