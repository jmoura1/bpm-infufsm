...
public class SecureWSConnectorCXF_2_4_0 extends ProcessConnector {
	...
	protected void executeConnector() throws Exception {
		...
		this.sourceResponse = dispatch.invoke(new StreamSource(new StringReader(enveloppe)));
		...
		if (isBuildResponseDocumentEnveloppe() || isBuildResponseDocumentBody()) {
			...
			if (result.getNode() instanceof Document) {
				this.responseDocumentEnveloppe = (Document) result.getNode();  
			} else {
				this.responseDocumentEnveloppe = result.getNode().getOwnerDocument();  
			}
		}

		if (isBuildResponseDocumentBody()) {
			if (this.responseDocumentEnveloppe != null) {
				this.responseDocumentBody = DocumentBuilderFactory.newInstance().newDocumentBuilder().newDocument();
				final Node bodyContent = getEnveloppeBodyContent(this.responseDocumentEnveloppe);
				final Node clonedBodyContent = bodyContent.cloneNode(true);
				this.responseDocumentBody.adoptNode(clonedBodyContent);
				this.responseDocumentBody.importNode(clonedBodyContent, true);
				this.responseDocumentBody.appendChild(clonedBodyContent);
			}
		}
	...
	private Node getEnveloppeBodyContent(final Document enveloppe) {
		...
		final Node bodyContentNode = enveloppeBody.getFirstChild();
	...
	public void setPrintRequestAndResponse(boolean printRequestAndResponse) {
