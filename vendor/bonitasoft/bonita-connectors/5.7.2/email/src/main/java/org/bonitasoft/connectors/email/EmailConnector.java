...
public class EmailConnector extends ProcessConnector {
   ...
   * 
   * @param host The name or IP address of the SMTP server
   */
   ...
   * 
   * @param ssl true if the SMTP server supports SSL.
   */
   ...
   * 
   * @param starttls true if the SMTP server supports STRARTTLS.
   */
   ...
   * 
   * @param pwd the password used for authentication
   */
   ...
   * 
   * @param to the "to" recipient(s) email address(es) separated by commas
   */
   ...
   * 
   * @param cc the "cc" recipient(s) email address(es) separated by commas
   */
   ...
   * 
   * @param bcc the "bcc" recipient(s) email address(es) separated by commas
   */
   ...
   * 
   * @param html true if the message is in HTML format
   */
   ...
   * 
   * @param attachments file paths of each attachment
   */
   ...
   * 
   * @param images HashMap containing image file paths and aliases
   */
  ...
  protected List<ConnectorError> validateValues() {
    ...
    } else if (this.getSmtpPort() > 65535) {
    ...
    if (this.getFrom() != null && this.getFrom().length() > 0) {
      error = checkAddress(this.getFrom());
      if (error != null) {
        errors.add(error);
      }
    }
    if (this.getTo().length() > 0) {
      error = checkAddresses("to", this.getTo());
      if (error != null) {
        errors.add(error);
      }
    }
    if (this.getCc() != null && this.getCc().length() > 0) {
      error = checkAddresses("cc", this.getCc());
      if (error != null) {
        errors.add(error);
      }
    }
    if (this.getBcc() != null && this.getBcc().length() > 0) {
      error = checkAddresses("bcc", this.getBcc());
      if (error != null) {
        errors.add(error);
      }
    }
   ...
   * @param addresses the addresses to be checked
   ...
   * @throws MessagingException if a MessageException occurs
