...
public abstract class SugarCrmSoapClient extends ProcessConnector {
     ...
     * @throws Exception
     */
    public void initSugarCrmSoapClient(final String sugarsoapPort, final String user, final String password, final String applicationName) throws MalformedURLException, ServiceException, RemoteException, LoginException, Exception {

        final Sugarsoap service = new SugarsoapLocator();
        port = service.getsugarsoapPort(new java.net.URL(sugarsoapPort));
     ...
     * @throws Exception
     */
    public List<List<Object>> getEntryList(final String module, final String query, final String orderBy, final String[] fieldsToRetrieve, final Link_name_to_fields_array[] linkNameToFieldsArray) throws Exception {
        if (LOG.isLoggable(Level.INFO)) {
        ...
        final Get_entry_list_result_version2 resultEntryList = port.get_entry_list(sessionId, module, query, orderBy, offset, fieldsToRetrieve, linkNameToFieldsArray, maxResults, getDeleted);
    ...
    public void getModuleFields(final String moduleName) throws Exception {
        ...
        New_module_fields moduleFields = port.get_module_fields(sessionId, moduleName, null);
        Field[] fields = moduleFields.getModule_fields();
        for (int i = 0; i < fields.length; i++) {
            if (LOG.isLoggable(Level.INFO)) {
                LOG.info("Field = Name: " + fields[i].getName() + " -- Label: " + fields[i].getLabel() + " -- Type: " + fields[i].getType() + " -- Required: " + fields[i].getRequired());
            }
        }
    }
     ...
     * @throws Exception
     */
    public void getRelationships(final String moduleName, final String moduleId, final String relatedModule, final String relatedModuleQuery) throws Exception {
        if (LOG.isLoggable(Level.INFO)) {
            LOG.info("--- " + moduleName + ":" + moduleId + " relationships with " + relatedModule + " -----------------------------------------------------------");
        }
        final String linkFieldName = "";
        Link_name_to_fields_array[] relatedModuleLinkNameToFieldsArray = null;
        final String[] relatedFields = null;
        port.get_relationships(sessionId, moduleName, moduleId, linkFieldName, relatedModuleQuery, relatedFields, relatedModuleLinkNameToFieldsArray, 0);
    }
