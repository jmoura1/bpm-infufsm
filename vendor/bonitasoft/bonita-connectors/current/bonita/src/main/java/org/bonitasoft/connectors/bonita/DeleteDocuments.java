...
public class DeleteDocuments extends ProcessConnector {
    ...
    protected void executeConnector() throws Exception {
        ...
        for (Entry<String, String> entry : parameters.entrySet()) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("deleting documentUUID = " + documentUUID + " , isDeleteAllVersions = " + allVersions);
            }
    ...
    protected List<ConnectorError> validateValues() {
        ...
        for (String documentUUID : parameters.keySet()) {
            if (documentUUID.length() != 32) {
                error = new ConnectorError("documentUUID Error", new Exception("length of documentUUID should be 32"));
                errors.add(error);
            }
            String value = parameters.get(documentUUID);
            if (!"true".equalsIgnoreCase(value) && !"false".equalsIgnoreCase(value)) {
                error = new ConnectorError("allVersions field Error", new Exception("the field allVersions should only be 'true' or 'false'"));
                errors.add(error);
            }
            try {
