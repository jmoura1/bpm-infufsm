 ...
 * This connector does the search operation.
...
public class LdapConnector extends Connector {
  ...
  /**
   * The protocol used by the directory service. It can be
   * LDAP, LDAPS(SSL) or TLS.
   */
  ...
  private LdapDereferencingAlias derefAliases =
    LdapDereferencingAlias.ALWAYS;
  ...
  //output
  private List<List<LdapAttribute>> result =
    new ArrayList<List<LdapAttribute>>();
  ...
  protected void executeConnector() throws Exception {
    ...
    try {
      ...
      while (count > 0 && answer.hasMore()) {
        ...
        List<LdapAttribute> elements = new ArrayList<LdapAttribute>();
        while (enume.hasMore()) {
  ...
  protected List<ConnectorError> validateValues() {
    ...
    switch (getScope()) {
      case BASE: case ONELEVEL: case SUBTREE:
        break;
      default:
    ...
    if (getCertificatePath() != null) {
      ...
      if (!temp.exists()) {
        errors.add(new ConnectorError("certificatePath",
            new IllegalArgumentException("the given path does not refer to a real file!")));
      }
    }
    if (getSizeLimit() < 0) {
      errors.add(new ConnectorError("sizeLimit",
          new IllegalArgumentException("null or negative value is forbidden!")));
    }
    if (getTimeLimit() < 0) {
      errors.add(new ConnectorError("timeLimit",
          new IllegalArgumentException("null or negative value is forbidden!")));
    }
    if (getReferralHandling() == null) {
      errors.add(new ConnectorError("referralHandling",
          new IllegalArgumentException("is null!")));
    } else if (!getReferralHandling().equals("ignore")
        && !getReferralHandling().equals("follow")) {
      errors.add(new ConnectorError("referralHandling",
