...
public class SAPCallFunctionConnector extends SAPAbstractConnector {
  ...
  protected void executeConnector() throws Exception {
    ...
    final ParameterList tableParameterList = jcoFunction.getTableParameterList();

    if (inputParameters != null) {
      final ParameterList inputParameterList = jcoFunction.getImportParameterList();
      for(List<Object> row : inputParameters) {
        final String parameterType = (String) row.get(0);
        final String tableName = (String) row.get(1);
        final String parameterName = (String) row.get(2);
        final Object parameterValue = row.get(3);

        if (INPUT_STRUCTURE.equals(parameterType)) {
          final Structure structure = inputParameterList.getStructure(tableName);
    ...
    try {
      ...
      if (outputParameters != null) {
        final ParameterList outputParameterList = jcoFunction.getExportParameterList();
        for(List<String> row : outputParameters) {
          ...
          if (OUTPUT_STRUCTURE.equals(parameterType)) {
            final Structure structure = outputParameterList.getStructure(tableName);
      ...
      if (commitOnSuccess) {
        final Function commitFunction = createFunction(jcoRepository, SAPConstants.BAPI_TRANSACTION_COMMIT);
        executeFunction(jcoClient, commitFunction);
      }
    } catch (Throwable e) {
      if (rollbackOnFailure) {
        final Function rollbackFunction = createFunction(jcoRepository, SAPConstants.BAPI_TRANSACTION_ROLLBACK);
    ...
    if (inputParameters != null) {
      ...
      for (List<Object> row : inputParameters) {
        if (row.size() != EXPECTED_INPUT_COLUMN_NUMBER) {
          errors.add(new ConnectorError("inputParameters", new IllegalArgumentException(
    ...
    if (outputParameters != null) {
      ...
      for (List<String> row : outputParameters) {
        if (row.size() != EXPECTED_OUTPUT_COLUMN_NUMBER) {
          errors.add(new ConnectorError("outputParameters", new IllegalArgumentException(
