 ...
 * 
 */
public class GroupSelection extends AbstractItemSelection<BonitaUUID> {
  ...
  public void select(final GroupSelector aSelector) {
    ArrayList<BonitaUUID> theOldSelection = new ArrayList<BonitaUUID>(myItemSelection);
    List<BonitaUUID> itemList = myDataSource.getVisibleItems();
    myItemSelection.clear();
    if (itemList != null) {
      switch (aSelector) {
      case All:
        myItemSelection.addAll(itemList);
        break;
      case None:
