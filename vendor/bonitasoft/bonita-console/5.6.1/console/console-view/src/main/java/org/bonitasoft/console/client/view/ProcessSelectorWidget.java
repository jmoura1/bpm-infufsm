 ...
 * 
 */
public class ProcessSelectorWidget extends I18NComposite implements ModelChangeListener {
  ...
  protected final ProcessSelection myItemSelection;
  protected final ProcessDataSource myDataSource;

  /**
   * Default constructor.
   */
  public ProcessSelectorWidget(ProcessDataSource aDataSource) {
    super();
    myItemSelection = (ProcessSelection) aDataSource.getItemSelection();
    myDataSource = aDataSource;
    mySelectorChoices = new ArrayList<String>();
    for (ProcessSelector theSelector : ProcessSelector.values()) {
      mySelectorChoices.add(theSelector.name());
    }
    myItemSelection.addModelChangeListener(ProcessSelection.ITEM_SELECTION_PROPERTY, this);
    initContent();
  ...
  private void initContent() {
    myCheckBox.addClickHandler(new ClickHandler() {
      ...
      public void onClick(ClickEvent aEvent) {
        if (myCheckBox.getValue()) {
          myItemSelection.select(ProcessSelector.All);
        } else {
          myItemSelection.select(ProcessSelector.None);
        }
    ...
    for (final String theSelectorChoice : mySelectorChoices) {
      theSubMenuChoices.addChoice(theSubMenuChoices.new MenuChoice(new Label(theSelectorChoice), new Command() {
        ...
        public void execute() {
          myItemSelection.select(ProcessSelector.valueOf(theSelectorChoice));
        }
      }));
    }
