...
/**
 * A basic widget that translates the prioritty of a step into a graphical flag.
 * The graphical flag is managed by a css style. This will ease the
 * customization.
 * 
 * @author Nicolas Chabanoles
 * 
 */
public class StepPriorityWidget extends BonitaPanel implements ModelChangeListener {
   ...
   * @param aStep
   */
  public StepPriorityWidget(StepItem aStep) {
    super();
  ...
  private void update() {
    ...
    myPlaceHolderPicture.setStyleName(CSSClassManager.getPriorityIconStyle(myStep.getPriority().name()));
    myPlaceHolderPicture.setTitle(patterns.stepDescriptionPriority(LocaleUtil.getPriorityLabel(myStep.getPriority())));
  }
  ...
  public void modelChange(ModelChangeEvent anEvent) {
    if (StepItem.PRIORITY_PROPERTY.equals(anEvent.getPropertyName())) {
      // Update the UI.
      update();
    }
  }
