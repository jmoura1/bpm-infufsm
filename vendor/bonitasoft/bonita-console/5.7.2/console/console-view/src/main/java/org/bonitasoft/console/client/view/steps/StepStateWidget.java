 ...
 * 
 */
public class StepStateWidget extends BonitaPanel implements ModelChangeListener {
	 ...
	 * @param aStep
	 */
	public StepStateWidget(StepItem aStep) {
		super();
	...
	private void update() {
		...
		myPlaceHolderPicture.setStyleName(CSSClassManager.getStepStateIconStyle(myStep.getState()));
		myPlaceHolderPicture.setTitle(patterns.stepState(LocaleUtil.translate(myStep.getState())));
	}
	...
	public void modelChange(ModelChangeEvent anEvent) {
		if (StepItem.STATE_PROPERTY.equals(anEvent.getPropertyName())) {
			// Update the UI.
			update();
		}
	}
