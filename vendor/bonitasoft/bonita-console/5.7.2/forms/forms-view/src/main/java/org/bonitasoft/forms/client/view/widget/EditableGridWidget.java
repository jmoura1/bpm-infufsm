...
public class EditableGridWidget extends Composite implements HasValueChangeHandlers<List<List<String>>> {
     ...
     * @param selectedItems
     */
    public EditableGridWidget(final String formID, final FormWidget widgetData, final List<List<String>> value) {

        this.formID = formID;
        this.widgetData = widgetData;
        
        if (widgetData.getMaxColumnsExpression() != null) {
            try {
                maxColumns = Integer.valueOf(widgetData.getMaxColumnsExpression());
            } catch (final NumberFormatException e) {
                Window.alert("The max number of columns has to be set with an integer or a groovy expression returning an integer.");
            }
        }
        if (widgetData.getMinColumnsExpression() != null) {
            try {
                minColumns = Integer.valueOf(widgetData.getMinColumnsExpression());
            } catch (final NumberFormatException e) {
                Window.alert("The min number of columns has to be set with an integer or a groovy expression returning an integer.");
            }
        }
        ...
        if (widgetData.getMinRowsExpression() != null) {
            try {
                minRows = Integer.valueOf(widgetData.getMinRowsExpression());
            } catch (final NumberFormatException e) {
                Window.alert("The min number of rows has to be set with an integer or a groovy expression returning an integer.");
            }
        }
