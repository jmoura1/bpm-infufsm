...
import org.bonitasoft.console.client.view.GroupSelectorWidget;
 ...
 * 
 */
public class GroupsListWidget extends AbstractItemList<BonitaUUID, Group, GroupFilter> {
  ...
  protected final HashMap<BonitaUUID, Label> myItemPathWidgets = new HashMap<BonitaUUID, Label>();
   ...
   */
  public GroupsListWidget(MessageDataSource aMessageDataSource, GroupDataSource aGroupDataSource) {
    super(aMessageDataSource, aGroupDataSource.getItemSelection(), aGroupDataSource, 20, 20, 5);
    myBonitaDataSource.addModelChangeListener(GroupDataSource.ITEM_LIST_PROPERTY, this);
    myBonitaDataSource.addModelChangeListener(GroupDataSource.ITEM_UPDATED_PROPERTY, this);
    ...
    theTopMenu.addItem(constants.delete(), new Command() {
      public void execute() {
          ...
          if (myItemSelection.getSize() > 0) {
             confirmationDialogbox = new ConfirmationDialogbox(constants.deleteGroupsDialogbox(), patterns.deleteGroupsWarn( myItemSelection.getSelectedItems().size()), constants.okButton(), constants.cancelButton());
  ...
  protected void deleteSelectedItems() {
    if (myItemSelection.getSize() > 0) {
      myBonitaDataSource.deleteItems(myItemSelection.getSelectedItems(), new AsyncHandler<ItemUpdates<Group>>() {
        public void handleFailure(Throwable anT) {
          myItemSelection.clearSelection();
        }

        public void handleSuccess(ItemUpdates<Group> anResult) {
          myItemSelection.clearSelection();
        }
      });
    } else {
...
//    if (anItem != null) {
//      final GroupEditorWidget theGroupEditorPanel = new GroupEditorWidget((GroupDataSource) myBonitaDataSource, anItem);
//      theGroupEditorPanel.addCancelClickHandler(new ClickHandler() {
//
//        public void onClick(ClickEvent event) {
//          addUpdateItemDialogBox.hide();
//        }
//      });
//      theGroupEditorPanel.addSaveHandler(new AsyncHandler<Void>() {
//
...
//          if (theItemRow != null) {
//            Group theItem = myBonitaDataSource.getItem(anItem);
//            myInnerTable.setWidget(theItemRow, 0, myItemSelectionWidgets.get(theItem.getUUID()));
//            myInnerTable.setWidget(theItemRow, 1, myItemNameWidgets.get(theItem.getUUID()));
//            myInnerTable.setWidget(theItemRow, 2, myItemLabelWidgets.get(theItem.getUUID()));
//            myInnerTable.setWidget(theItemRow, 3, myItemPathWidgets.get(theItem.getUUID()));
//            myInnerTable.setWidget(theItemRow, 4, myItemDescriptionWidgets.get(theItem.getUUID()));
//          }
//        }
//      });
//      addUpdateItemDialogBox.clear();
//      addUpdateItemDialogBox.add(theGroupEditorPanel);
//      addUpdateItemDialogBox.setText(constants.groupUpdate());
//      addUpdateItemDialogBox.center();
//      addUpdateItemDialogBox.show();
//      theGroupEditorPanel.setFocus();
//    }
//  }

  protected void addItem() {
    final GroupEditorWidget theGroupEditorPanel = new GroupEditorWidget((GroupDataSource) myBonitaDataSource, null);
    theGroupEditorPanel.addCancelClickHandler(new ClickHandler() {
    ...
    addUpdateItemDialogBox.clear();
    addUpdateItemDialogBox.add(theGroupEditorPanel);
    addUpdateItemDialogBox.setText(constants.groupCreation());
    addUpdateItemDialogBox.center();
    theGroupEditorPanel.setFocus();
  }
  ...
  protected void createWidgetsForItemsAndDisplay() {
    ...
    myVisibleItems = ((GroupDataSource) myBonitaDataSource).getVisibleItems();
  ...
  protected void createWidgetsForItem(BonitaUUID anItem) {
    ...
    myItemPathWidgets.put(anItem, new Label(Group.buildGroupPath(theItem)));
  }
  
  protected void updateWidgetsForItem(Group anItem) {
      ...
      myItemPathWidgets.get(anItem.getUUID()).setText(Group.buildGroupPath(anItem));
  }
  ...
  public void notifyItemClicked(BonitaUUID anItem, final ClickEvent anEvent) {
    ...
    if (theCell.getCellIndex() > 0 && anItem != null) {
        History.newItem(ViewToken.GroupEditor.name() + ConsoleConstants.TOKEN_SEPARATOR + anItem.getValue());
    }
  }
  ...
  protected void update(List<BonitaUUID> anItemList) {
    ...
    myInnerTable.setWidget(1, 1, new Label(constants.groupPathElement()));
    myInnerTable.setWidget(1, 2, new Label(constants.label()));
    myInnerTable.setWidget(1, 3, new Label(constants.groupPath()));
    myInnerTable.setWidget(1, 4, new Label(constants.description()));
  ...
  protected void fillInContentRow(List<BonitaUUID> anItemList) {
    ...
    for (BonitaUUID theItemUUID : anItemList) {
      ...
      myInnerTable.setWidget(theCurrentRowIndex, 3, myItemPathWidgets.get(theItem.getUUID()));
      myInnerTable.setWidget(theCurrentRowIndex, 4, myItemDescriptionWidgets.get(theItem.getUUID()));
  ...
  public void modelChange(ModelChangeEvent anEvent) {
    ...
    if (GroupDataSource.ITEM_LIST_PROPERTY.equals(anEvent.getPropertyName())) {
      ...
      myBonitaDataSource.reload();
    }  else if(GroupDataSource.ITEM_UPDATED_PROPERTY.equals(anEvent.getPropertyName())) {
      BonitaUUID theItemUUID = (BonitaUUID)anEvent.getSource();
      if(myItemSelectionWidgets.containsKey(theItemUUID)) {
        Group theItem = (Group)anEvent.getNewValue();
  ...
  @Override
  protected ItemFilterEditor<GroupFilter> buildFilterEditor() {
    SimpleFilterEditor<GroupFilter> theEditor = new SimpleFilterEditor<GroupFilter>(myMessageDataSource, myBonitaDataSource.getItemFilter(), constants.filterGroupsToolTip());
