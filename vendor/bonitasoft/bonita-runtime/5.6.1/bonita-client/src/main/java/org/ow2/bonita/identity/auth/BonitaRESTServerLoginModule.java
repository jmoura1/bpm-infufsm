...
public class BonitaRESTServerLoginModule implements LoginModule {
  ...
  private void processUsersPasswordsAndRoles() throws BadLoginOptionException, MissingOptionException {
    if (this.logins == null) {
      final String loginList = (String) this.options
          .get(BonitaRESTServerLoginModule.LOGINS_OPTION_NAME);
      final String passwordList = (String) this.options
          .get(BonitaRESTServerLoginModule.PASSWORDS_OPTION_NAME);
      final String rolesList = (String) this.options
      .get(BonitaRESTServerLoginModule.ROLES_OPTION_NAME);
      if (loginList == null) {
        String message = ExceptionManager.getInstance().getFullMessage(
            "bi_PLM_1", BonitaRESTServerLoginModule.LOGINS_OPTION_NAME);
        throw new MissingOptionException("bi_PLM_1", message);
      }
      if (passwordList == null) {
        String message = ExceptionManager.getInstance().getFullMessage(
            "bi_PLM_2", BonitaRESTServerLoginModule.PASSWORDS_OPTION_NAME);
        throw new MissingOptionException("bi_PLM_2", message);
      }

      if (rolesList == null) {
        String message = ExceptionManager.getInstance().getFullMessage(
            "bi_PLM_7", BonitaRESTServerLoginModule.ROLES_OPTION_NAME);
      ...
      if (loginTokenizer.countTokens() != passwordTokenizer.countTokens()) {
        String message = ExceptionManager.getInstance().getFullMessage("bi_PLM_8");
  ...
  public boolean commit() throws LoginException {
    if (this.id == null) {
      String message = ExceptionManager.getInstance().getFullMessage("bi_PLM_6");
      throw new FailedLoginException(message);
    }
    ...
    if (this.id != null) {
      if (this.debug) {
        System.err.println("[" + BonitaRESTServerLoginModule.class.getName()
            + "] logout() - removing principals");
      }
      // Remove only principals added by our commit method
      final Set<Principal> principals = new HashSet<Principal>(this.subject
          .getPrincipals());
      ...
      final Set<Object> credentials = new HashSet<Object>(this.subject
