...
public class FileStreamSource extends StreamSource {
  ...
  public FileStreamSource(File file) {
    if (file == null) {
    	String message = ExceptionManager.getInstance().getFullMessage("bp_FSS_1");
  ...
  public InputStream openStream() {
    ...
    try {
      if (!file.exists()) {
      	String message = ExceptionManager.getInstance().getFullMessage("bp_FSS_2", file);
        throw new BonitaRuntimeException(message);
      }
      if (file.isDirectory()) {
      	String message = ExceptionManager.getInstance().getFullMessage("bp_FSS_3", file);
        throw new BonitaRuntimeException(message);
      }
