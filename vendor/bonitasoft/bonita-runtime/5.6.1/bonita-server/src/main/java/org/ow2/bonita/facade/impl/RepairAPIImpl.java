...
public class RepairAPIImpl implements RepairAPI {
  ...
  public ProcessInstanceUUID copyProcessInstance(ProcessInstanceUUID instanceUUID, Map<String, Object> processVariables, Collection<InitialAttachment> attachments) throws InstanceNotFoundException, VariableNotFoundException{
    return copyProcessInstance(instanceUUID, processVariables, attachments, null);
  }
  
  public ProcessInstanceUUID copyProcessInstance(ProcessInstanceUUID instanceUUID, Map<String, Object> processVariables, Collection<InitialAttachment> attachments, Date restoreVariableValuesAtDate) throws InstanceNotFoundException, VariableNotFoundException {
    ...
    final InternalProcessDefinition processDefinition = EnvTool.getJournalQueriers().getProcess(processInstance.getProcessDefinitionUUID());
    
    List<String> activitiesNames = new ArrayList<String>();
    Set<Execution> activiesExecutions = searchActiveExecutions(processInstance.getRootExecution(), null, Execution.STATE_ACTIVE);
    ...
    if (processInstance.getNbOfAttachments() > 0 ) {
      ...
      final List<AttachmentInstance> attachmentInstances = DocumentService.getAllAttachmentVersions(manager, instanceUUID);

      for (AttachmentInstance attachmentInstance : attachmentInstances) {
        if (restoreVariableValuesAtDate == null || attachmentInstance.getVersionDate().getTime() <= restoreVariableValuesAtDate.getTime()) {
          ...
          final AttachmentDefinitionImpl attachmentDefinition = new AttachmentDefinitionImpl(definitionUUID, attachmentName);
          ...
          final SearchResult result = DocumentService.getDocuments(manager, definitionUUID, attachmentName);
    ...
    try {
      processInstanceCopyUUID = instantiateProcess(processInstance.getProcessDefinitionUUID(), variablesToSet, attachmentsToSet.values(), activitiesNames, instanceInitiator);
  ...
  public ProcessInstanceUUID instantiateProcess(ProcessDefinitionUUID processDefinitionUUID,
      Map<String, Object> processVariables, Collection<InitialAttachment> attachments, List<String> startActivitiesNames) throws ProcessNotFoundException,
      VariableNotFoundException, ActivityNotFoundException {
    return instantiateProcess(processDefinitionUUID, processVariables, attachments, startActivitiesNames, EnvTool.getUserId());
  }

  public ProcessInstanceUUID instantiateProcess(ProcessDefinitionUUID processDefinitionUUID,
      Map<String, Object> processVariables, Collection<InitialAttachment> attachments,
      List<String> startActivitiesNames, String instanceInitiator) throws ProcessNotFoundException,
      VariableNotFoundException, ActivityNotFoundException {
    FacadeUtil.checkArgsNotNull(processDefinitionUUID);
    if (LOG.isLoggable(Level.FINE)) {
      LOG.fine("Starting a new instance of process : " + processDefinitionUUID);
    }
    Execution rootExecution = ProcessUtil.createProcessInstance(processDefinitionUUID, processVariables, attachments, null, null, null, null);
    if (LOG.isLoggable(Level.FINE)) {
    ...
    if (startActivitiesNames != null) {
      Set<String> executionsAlreadyStarted = new HashSet<String>();
      for (String activityName : startActivitiesNames) {
        //make sure there are no attempts to start several executions of the same activity
  ...
  public ActivityInstanceUUID startExecution(ProcessInstanceUUID instanceUUID, String activityName)
    ...
    Execution execution = new Execution(activityName, processDefinition, processInstance, activityDefinition, Execution.STATE_CREATED, null);
  ...
  public void stopExecution(ProcessInstanceUUID instanceUUID, String activityName) throws
    ...
    Set<Execution> activiesExecutions = searchActiveExecutions(processInstance.getRootExecution(), activityName, null);
    for (Execution execution : activiesExecutions) {
