...
public class RESTVariableTest  extends RESTAPITestCase {
  public void testGetBooleanVariable() throws Exception {
    ...
    String xBVariable = ApacheHttpClientUtil.getResponseContent(getProcessInstanceVariableResponse);
    
    assertNotNull(xBVariable);
    Boolean bVariable = (Boolean) xstream.fromXML(xBVariable);
  ...
  public void testSpecialCaracterOnVariables() throws Exception {
    ...
    variables.put("strVariable", URLEncoder.encode("%", "UTF-8"));
    String xmlVariables = xstream.toXML(variables);
  ...
  public void testCustomObjects() throws Exception {
    ...
    HttpResponse lightActivitiesResponse = ApacheHttpClientQueryRuntimeAPI.getLightActivities(instanceUUID.toString(), getUserForTheOptionsMap(), REST_USER, REST_PSWD);
    assertEquals(Status.OK.getStatusCode(), lightActivitiesResponse.getStatusLine().getStatusCode());
    String strActivities = ApacheHttpClientUtil.getResponseContent(lightActivitiesResponse);
