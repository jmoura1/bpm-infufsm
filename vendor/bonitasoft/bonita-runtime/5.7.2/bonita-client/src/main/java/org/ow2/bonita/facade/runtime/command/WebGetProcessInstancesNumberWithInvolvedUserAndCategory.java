  ...
  private final String username;
    ...
    case ALLOW_BY_DEFAULT:
      ...
      if (processUUIDs != null && !processUUIDs.isEmpty()) {
        Integer result;
        result = queryRuntimeAPI.getNumberOfParentProcessInstancesWithInvolvedUserAndCategoryExcept(username, category, processUUIDs);
        return result;
      } else {
        Integer result;
    ...
    case DENY_BY_DEFAULT:
      ...
      if (processUUIDs.size() > 0) {
        Integer result;
        result = queryRuntimeAPI.getNumberOfParentProcessInstancesWithInvolvedUserAndCategory(username, category, processUUIDs);
        return result;
      } else {
        return 0;
      }
