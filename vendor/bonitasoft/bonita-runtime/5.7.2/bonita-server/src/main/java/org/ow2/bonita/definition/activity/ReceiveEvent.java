...
public class ReceiveEvent extends AbstractActivity {
  ...
  protected boolean executeBusinessLogic(final Execution execution) {
  	...
  	IncomingEventInstance incomingEventInstance = new IncomingEventInstance(incomingEvent.getName(), incomingEvent.getExpression(), instanceUUID,
  	    activity.getUUID(), activityUUID, execution.getProcessDefinition().getName(), activityName,
  	    eventUUID, RECEIVE_EVENT_SIGNAL, System.currentTimeMillis(), true);
  	
  	if (activity.getType().equals(Type.ReceiveEvent) && activity.getIncomingTransitions().isEmpty()) {
      final IncomingEventDefinition event = activity.getIncomingEvent();
      if (event != null) {
        incomingEventInstance.setLockOwner(instanceUUID.toString());
      }
    }
