...
public class WireContext extends DefaultObservable implements Context, Closable, Serializable {
   ...
   * 
   * @param delayCreate specifies if creation should be postponed till {@link #create()} is called explicitly. If
   *          delayCreate is set to false, creation is done as part of the constructor. If delayCreate is set to true,
   *          the {@link #create()} method needs to be called explicitly by the client after construction is complete.
   *          The use case is creation of environment where the transactionName needs to be set and the scope needs to
   *          be added to the environment before the creation of this wire scope is done.
   * @see PvmEnvironmentFactory#openEnvironment()
   */
  public WireContext(final WireDefinition wireDefinition, final String name, final Environment environment,
      final boolean delayCreate) {
    super();
    this.wireDefinition = wireDefinition;
    this.name = name;
    this.classLoader = wireDefinition != null ? wireDefinition.getClassLoader() : null;
   ...
   * 
   * @param isDelayable indicates wether initialization is delayable. When isDelayable is set to false the returned
   *          object will be constructed and initialized. When isDelayable is set to true, the returned object will be
   *          constructed, but not necessarily initialized.
