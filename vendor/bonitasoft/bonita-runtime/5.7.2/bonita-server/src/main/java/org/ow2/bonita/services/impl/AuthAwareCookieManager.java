...
public class AuthAwareCookieManager extends CookieManager {
   ...
   * 
   * @param store a <tt>CookieStore</tt> to be used by cookie manager. if <tt>null</tt>, cookie manager will use a
   *          default one, which is an in-memory CookieStore implmentation.
   * @param cookiePolicy a <tt>CookiePolicy</tt> instance to be used by cookie manager as policy callback. if
   ...
   * 
   * <p>
   * A instance of <tt>CookieManager</tt> will have cookie policy ACCEPT_ORIGINAL_SERVER by default. Users always can
   * call this method to set another cookie policy.
   * 
  ...
  public void put(final URI uri, final Map<String, List<String>> responseHeaders) throws IOException {
    ...
    for (final Entry<String, List<String>> header : responseHeaders.entrySet()) {
  ...
  /*
   * sort cookies with respect to their path: those with more specific Path attributes precede those with less specific,
   * as defined in RFC 2965 sec. 3.3.4
   */
