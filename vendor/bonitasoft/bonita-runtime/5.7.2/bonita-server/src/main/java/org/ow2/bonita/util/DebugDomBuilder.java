...
/**
 * builds the dom model from SAX events, optionally adding the line and column
 * number as attributes to every element.
 */
class DebugDomBuilder extends DefaultHandler implements ContentHandler,
    LexicalHandler {
  /** Root document */
    ...
    if (null != currentNode) {
      ...
      if (type == Node.TEXT_NODE) {
        ...
        if ((null != data) && (data.trim().length() > 0)) {
        	String message = ExceptionManager.getInstance().getFullMessage("bp_DDB_1");
        ...
        if (this.document.getDocumentElement() != null) {
          ok = false;
          String message = ExceptionManager.getInstance().getFullMessage("bp_DDB_2");
          throw new org.xml.sax.SAXException(message);
        }
      }
      ...
      if (locator != null) {
        ...
        if (debugNamespace == null) {
          ...
          if (columnAttributeName != null) {
            elem.setAttribute(columnAttributeName, Integer
                .toString(columnNumber));
          }

        } else {
          if (lineAttributeName != null) {
            elem.setAttributeNS(debugNamespace, lineAttributeName, Integer
                .toString(lineNumber));
          }
          if (columnAttributeName != null) {
            elem.setAttributeNS(debugNamespace, columnAttributeName, Integer
                .toString(columnNumber));
          }
  ...
  public void endElement(String ns, String localName, String name)
    ...
    this.elemStack.pop();
    this.currentNode = this.elemStack.isEmpty() ? null : (Node) this.elemStack
        .peek();
  }
  ...
  public void characters(char ch[], int start, int length)
    ...
    Node childNode;
    childNode = this.currentNode != null ? this.currentNode.getLastChild()
        : null;
  ...
  public void charactersRaw(char ch[], int start, int length)
    ...
    append(this.document.createProcessingInstruction("xslt-next-is-raw",
        "formatter-to-dom"));
  ...
  private boolean isOutsideDocElem() {
    return (null == this.docFrag)
        && this.elemStack.size() == 0
        && (null == this.currentNode || this.currentNode.getNodeType() == Node.DOCUMENT_NODE);
  }
