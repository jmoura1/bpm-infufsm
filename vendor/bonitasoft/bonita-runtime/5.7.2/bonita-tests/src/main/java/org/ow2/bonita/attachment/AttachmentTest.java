...
public class AttachmentTest extends APITestCase {
  ...
  public void testBigAttachmentDefinition() throws Exception {
      long l = 40000;
      long expectedMinSize = 1000 * 1000;
      File f = generatePropertiesFile(l);
      ...
      byte[] documentContent = getQueryRuntimeAPI().getDocumentContent(document.getUUID());
  ...
  public void testAttachmentDefinition() throws Exception {
    ...
    AttachmentDefinition attachmentB = process.getAttachment("b");
    //assertEquals("b", attachmentB.getFileName());
    InitialAttachment processAttachment = getQueryDefinitionAPI().getProcessAttachment(processUUID, "b");
    ...
    AttachmentInstance attachment = getQueryRuntimeAPI().getLastAttachment(instanceUUID, "b");
    ...
    byte[] content = getQueryRuntimeAPI().getAttachmentValue(attachment);
  ...
  public void testGetAttachemnts() throws Exception {
    ...
    Set<InitialAttachment> attachements = getQueryDefinitionAPI().getProcessAttachments(processUUID);
    assertEquals(4, attachements.size());

    InitialAttachment attachement = getQueryDefinitionAPI().getProcessAttachment(processUUID, "b");
  ...
  public void testGetInstanceAttachmentInGroovy() throws Exception {
    ...
    try {
      ...
      getRuntimeAPI().executeTask(activityInstanceUUID, true);
      String actual = (String) getQueryRuntimeAPI().getProcessInstanceVariable(instanceUUID, "nothing");
  ...
  private File generatePropertiesFile(long additionalProperties) throws IOException {
    Properties properties = new Properties();
    properties.put("a", "1");
    properties.put("b", "2");
    properties.put("c", "3");
  ...
  public void testRemoveAnAttachment() throws Exception {
    ...
    byte[] initialValue = new byte[] {1, 2, 3, 4, 5, 6};
    Map<String, byte[]> resources = new HashMap<String, byte[]>();
  ...
  public void testByteArrayAttachmentWithSetVariable() throws Exception {
    ...
    byte[] attachmentValue1 = getQueryRuntimeAPI().getAttachmentValue(attachments.get(0));
    byte[] attachmentValue2 = getQueryRuntimeAPI().getAttachmentValue(attachments.get(1));
  ...
  public void testAttachementInstanceWithSetVariable() throws Exception {
    byte[] initialValueC = new byte[] {3, 2};
    byte[] initialValueD = new byte[] {5, 4};
    Map<String, byte[]> resources = new HashMap<String, byte[]>();
  ...
  public void testAttachemntsWithSetVariableActivityVariableAndAttachmentWithTheSameName() throws Exception {
    byte[] initialValue = new byte[] {3, 2};
    Map<String, byte[]> resources = new HashMap<String, byte[]>();
    resources.put("attachB", initialValue);
    ...
    attachments = getQueryRuntimeAPI().getAttachments(instanceUUID, "b");
    assertEquals(1, attachments.size());
    byte[] attachmentValue = getQueryRuntimeAPI().getAttachmentValue(attachments.get(0));
    assertEquals(2, attachmentValue.length);
    assertEquals(3, attachmentValue[0]);
    assertEquals(2, attachmentValue[1]);

    String variableValue = (String)getQueryRuntimeAPI().getActivityInstanceVariable(activityUUID, "b");
  ...
  public void testGetInstanceAttachmentInitialFileName() throws Exception {
    ...
    BusinessArchive businessArchive = BusinessArchiveFactory.getBusinessArchive(attachmentProcess,parentResources);
    attachmentProcess = getManagementAPI().deploy(businessArchive);
    ...
    try {
      ...
      AttachmentInstance attachment = getQueryRuntimeAPI().getLastAttachment(instanceUUID, "attachment");
  ...
  public void testDeployUndeployRedeploy() throws Exception {
    ...
    BusinessArchive businessArchive = getBusinessArchive(process, resources);
    ...
    WebDeleteProcessCommand webDeleteProcessCommand = new WebDeleteProcessCommand(processUUID);
  ...
  public void testGroovyAttachment() throws Exception {
    ...
    byte[] initialValue = firstContent.getBytes();
    final String secondContent = "secondContent";
    byte[] newValue = secondContent.getBytes();
    ...
    byte[] actual = (byte[]) getRuntimeAPI().evaluateGroovyExpression(groovyScript, instanceUUID, false);
  ...
  public void testNullFileNamettachment() throws Exception {
    ...
    AttachmentDefinition attachmentDefinition = getQueryDefinitionAPI().getAttachmentDefinition(attachmentProcess.getUUID(), attachmentName);
