...
public class LocalEventsTest extends APITestCase {
  ...
  public void testCreateInstanceEventWithFailingConnector() throws Exception {
    getManagementAPI().deployJar("commands.jar", Misc.generateJar(GetOutgoingEventInstancesUUIDsCommand.class, GetNonPermanentIncomingEventInstancesRetriesCommand.class));
    ...
    .addReceiveEventTask("receive", "createProcess")
      .addReceiveEventConnector(FailingConnector.class.getName(), true)
    .done();
    ...
    assertEquals(1, outgoingUUIDs.size());
    assertEquals(creatorInstanceUUID, outgoingUUIDs.iterator().next());

    final Set<IncomingRetry> retries = getCommandAPI().execute(new GetNonPermanentIncomingEventInstancesRetriesCommand());
    ...
    final Set<ProcessInstanceUUID> outgoingUUIDs2 = getCommandAPI().execute(new GetOutgoingEventInstancesUUIDsCommand());
    assertEquals(0, outgoingUUIDs2.size());

    final Set<IncomingRetry> retries2 = getCommandAPI().execute(new GetNonPermanentIncomingEventInstancesRetriesCommand());
