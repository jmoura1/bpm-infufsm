...
public class CacheTest extends APITestCase {
    ...
    private ProcessDefinition getAWMultiInstProcess(final int i) {
        final ProcessDefinition process = ProcessBuilder
                ...
                .addStringData("MyRequest").addGroup("User").addGroupResolver(InstanceInitiator.class.getName()).addHuman("human").addSystemTask("BonitaEnd")
                .addJoinType(JoinType.XOR).addSystemTask("BonitaStart").addHumanTask("Approval", "human").addStringData("decisionAccepted")
                .addStringData("performer", "nobody").addFilter(UserPerformerAssign.class.getName())
                .addMultiInstanciation("performer", ApprovalInstantiator.class.getName()).addSystemTask("CheckDecision").addStringData("decisionAccepted")
                .addConnector(Event.automaticOnEnter, CheckDecision.class.getName(), true).addSystemTask("Accept")
                .addConnector(Event.automaticOnEnter, Accept.class.getName(), true).addSystemTask("Reject")
                .addConnector(Event.automaticOnEnter, Reject.class.getName(), true).addAttachment("myDoc" + i, "/home/manu/myDocPath" + i)
                .addTransition("Start_Approval", "BonitaStart", "Approval").addTransition("Approval_CheckDecision", "Approval", "CheckDecision")
                .addTransition("CheckDecision_Reject", "CheckDecision", "Reject").addCondition("decisionAccepted.compareTo(\"no\") == 0")
                .addTransition("CheckDecision_Accept", "CheckDecision", "Accept").addCondition("decisionAccepted.compareTo(\"yes\") == 0")
                .addTransition("Reject_End", "Reject", "BonitaEnd").addTransition("Accept_End", "Accept", "BonitaEnd").done();
        return process;
    }
    ...
    private void getProcessDefinition(final ProcessDefinitionUUID uuid, final int nTimes) throws BonitaException {
        ...
        // System.out.println("FIRST getProcessDefinition(" + uuid + " in (ms): " + firstCallDuration);

        List<Long> execTimes = new ArrayList<Long>(nTimes);
    ...
    public void testLoopDeployAndGetProcessDef() throws BonitaException {
        ...
        List<ProcessDefinitionUUID> uuids = new ArrayList<ProcessDefinitionUUID>(50);
